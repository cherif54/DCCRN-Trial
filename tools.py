...
def find_pair(noisy_dirs):
    ...
    for i in range(len(noisy_dirs)):
        ...
        if addrs.endswith(".wav"):
            ...
            #clean_dirs.append(addr_clean) #changed to take raw file name
            print("find_pairs ",addrs)
            clean_dirs.append(addrs)
    return clean_dirs
...
def truncator(file_directory,dir_name):

    file_dirs = scan_directory(file_directory)     
    num= len(file_dirs)    
    wl=cfg.wav_length
    ...
    for i in range(num):
        ...
        wav, fs = soundfile.read(addr)    
        aud_length= wl*fs
        winds=int(len(wav)/aud_length)+1
        wav.resize(winds*aud_length)
        
        newpath = os.getcwd()+"/"+dir_name
        if not os.path.exists(newpath):
            os.makedirs(newpath)
        
        
        for j in range(winds):
            fl=Path(addr).stem
            new_file=os.getcwd()+"/"+dir_name+"/"+fl+"_"+str(j)+".wav"
            #print(new_file)
            #print(aud_length*j, aud_length*(j+1))
            truncated=wav[aud_length*j : aud_length*(j+1)]
            write(new_file, fs, truncated)
    

    


def largest_file(f1,f2,f3,f4):
    f1=f1+"*"
    f2=f2+"*"
    f3=f3+"*"
    f4=f4+"*"
    folder1=glob.glob(f1)
    max_file_1 = max( folder1,key =  lambda x: os.stat(x).st_size)
    folder2=glob.glob(f2)
    max_file_2 = max( folder2,key =  lambda x: os.stat(x).st_size)
    folder3=glob.glob(f3)
    max_file_3 = max( folder3,key =  lambda x: os.stat(x).st_size)
    folder4=glob.glob(f4)
    max_file_4 = max( folder4,key =  lambda x: os.stat(x).st_size)
    
    wave_length1= len(addr2wav(max_file_1))
    wave_length2= len(addr2wav(max_file_2))
    wave_length3= len(addr2wav(max_file_3))
    wave_length4= len(addr2wav(max_file_4))
    wave_length= max(wave_length1,wave_length2,wave_length3,wave_length4)
    print("Largest file_length ", wave_length)
    with open('temporary_file_length.txt', 'w') as f:
        f.write(str(wave_length))


def test_concat(addrs):
    directory=scan_directory(addrs)
    num= len(directory)
    ln_start=0
    os.mkdir(cfg.test_output)
    for i in range(num):
        addr= directory[i]
        wav, fs = soundfile.read(addr)
        length= len(wav)
        ln_end=length+ln_start
        pth=os.getcwd()
        concat=pth+"/concatenated_file.wav"
        outwav, fs = soundfile.read(concat)
        wavseg=outwav[ln_start:ln_end]
        
        fl=Path(addr).stem
        file_name=cfg.test_output+"/"+fl+".wav"
        
        write(file_name,cfg.fs,wavseg)
...
def test_match(input_dir,output_dir):
    print("Writting output files...")
    directory=scan_directory(input_dir)
    truncated=scan_directory(output_dir)
    num=len(directory)
    wl=cfg.wav_length
    start_ind=0
    ...
    for i in range(num):
        ...
        fname=Path(addr).stem  
        output_file=cfg.test_output+"/"+fname+".wav"         
        for j in range(winds):
            index=j+start_ind
            twav, fs=soundfile.read(truncated[index])
            full_file.extend(twav)    
        full_file_np=np.array(full_file )
        write(output_file, cfg.fs, full_file_np)  
        start_ind=index+1     
...
#######################################################################
#######################################################################
def pesq_arr(estimated_wavs, clean_wavs):
    #pesq_num=np.array([])
    pesq_num=[]
    for i in range(len(estimated_wavs)):
        pesq_score = pesq(cfg.FS, clean_wavs[i], estimated_wavs[i], "wb", on_error=PesqError.RETURN_VALUES)
        pesq_num=np.append(pesq_num,pesq_score)
    return pesq_num

def stoi_arr(estimated_wavs, clean_wavs):
    #stoi_num=np.array([])
    stoi_num=[]
    for i in range(len(estimated_wavs)):
        stoi_score = stoi(estimated_wavs[i], estimated_wavs[i], cfg.FS, extended=False)# pesq: 0.1 better / stoi: 0.01 better
        stoi_num=np.append(stoi_num,stoi_score)
    return stoi_num
...
def cal_pesq(dirty_wavs, clean_wavs):
    ...
    for i in range(len(dirty_wavs)):
        pesq_score = pesq(cfg.FS, clean_wavs[i], dirty_wavs[i], "wb", on_error=PesqError.RETURN_VALUES)
