...
def model_validate(model, validation_loader, dir_to_save, writer, epoch, DEVICE):
    ...
    with torch.no_grad():
        for inputs, labels in Bar(validation_loader):
            ...
            avg_pesq = cal_pesq(clean_wavs, estimated_wavs)  ## 98
            avg_stoi = cal_stoi(clean_wavs, estimated_wavs)
            
            pesq=pesq_arr(clean_wavs, estimated_wavs)
            stoi=stoi_arr(clean_wavs, estimated_wavs)
            print("INput PESQ shape and type", pesq.shape, type(pesq))
            ...
            print("INput PESQ shape and type after reshape", pesq.shape, type(pesq))
        ...
        if epoch % 10 == 0:
            ...
            print("all_batch_pesq shape ",len(all_batch_pesq),"shape", all_batch_pesq.shape, all_batch_pesq[4])
