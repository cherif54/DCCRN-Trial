...
"""
import torch
from tools import cal_pesq, cal_stoi, pesq_arr, stoi_arr
import numpy as np
from scipy.io.wavfile import write 
from tools_for_model import near_avg_index, max_index, min_index, Bar
from write_on_tensorboard import Writer as writer
from config import fs, info, mode
import torch.nn.functional as F
from pesq import pesq
import config as cfg
import os
from tools import scan_directory
...
def model_validate(model, validation_loader, dir_to_save, writer, epoch, DEVICE):
    ...
    with torch.no_grad():
        ...
        if epoch % 10 == 0:
            ...
            np.save("all_batch_pesq", all_batch_pesq)
...
def model_test(noise_type, snr, model, test_loader, dir_to_save, DEVICE):
    ...
    with torch.no_grad():
        ...
        for inputs, labels in Bar(test_loader):
            ...
            pesq = pesq_arr(estimated_wavs, clean_wavs)
            stoi = stoi_arr(estimated_wavs, clean_wavs)
...
def model_infer(model, test_loader, DEVICE):
    ...
    pth=os.getcwd()
    tdata=pth+"/test_data/"
    file_names=scan_directory(tdata)
    with torch.no_grad():
        # initialization
        batch_num = 0

        for inputs in Bar(test_loader):
            batch_num += 1
            ...
            mask_real, mask_imag, real_spec, img_spec, outputs = model(inputs)
            estimated_wavs = outputs.cpu().detach().numpy()
            wavnp=np.array(estimated_wavs)
            dimension=estimated_wavs.shape
            for i in range(dimension[0]):
            	index=cfg.batch*(batch_num-1)+i 
            	output_name=Path(file_names[index]).stem           
            	full_output_name=pth+"/temp_output/"+output_name+".wav" 
            	write(full_output_name,cfg.fs,wavnp[i])                       
            	#full_file.extend(estimated_wavs[i])
            	#wavname=file_names[]
            	
            
    #full_file_np=np.array(full_file)
    #write("concatenated_file.wav", cfg.fs, full_file_np)
