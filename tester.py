...
from tools import truncator, test_match
...
if cfg.test is True:
    ...
    pth=os.getcwd()
    temp_output=pth+"/temp_output/"
    os.makedirs(temp_output)
    os.makedirs(cfg.test_output)

    # check the lowest validation loss epoch
    want_to_check = torch.load(dir_to_save + '/chkpt_10.pt')
    ...
    test_loader = create_dataloader_for_infer('infer')
    model_infer(model, test_loader, DEVICE)
    
    test_match(cfg.test_folder,temp_output)
    test_dt=pth+"/test_data" 
    shutil.rmtree(test_dt)
